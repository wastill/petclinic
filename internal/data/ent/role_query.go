...
// QueryBelongUser chains the current query on the "belong_user" edge.
func (rq *RoleQuery) QueryBelongUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, role.BelongUserTable, role.BelongUserColumn),
		)
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withBelongUser: rq.withBelongUser.Clone(),
...
// WithBelongUser tells the query-builder to eager-load the nodes that are connected to
// the "belong_user" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithBelongUser(opts ...func(*UserQuery)) *RoleQuery {
	query := (&UserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withBelongUser = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	var (
		...
		_spec       = rq.querySpec()
		loadedTypes = [1]bool{
			rq.withBelongUser != nil,
		}
	)
	if rq.withBelongUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withBelongUser; query != nil {
		if err := rq.loadBelongUser(ctx, query, nodes, nil,
			func(n *Role, e *User) { n.Edges.BelongUser = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadBelongUser(ctx context.Context, query *UserQuery, nodes []*Role, init func(*Role), assign func(*Role, *User)) error {
